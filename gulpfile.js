...
import ts from './gulp/compileScripts.mjs';
...
const syncServer = () => {
  ...
  gulp.watch('source/ts/**/*.{ts,json}', gulp.series(ts, refresh));
...
const build = gulp.series(clean, copy, sprite, gulp.parallel(styles, ts, pug, optimizePng, optimizeJpg, optimizeSvg));
const dev = gulp.series(clean, copy, sprite, gulp.parallel(styles, ts, pug, optimizePng, optimizeJpg, optimizeSvg), syncServer);
const start = gulp.series(clean, copy, sprite, gulp.parallel(styles, ts, pug), syncServer);
const nomin = gulp.series(clean, copy, sprite, gulp.parallel(styles, ts, pug, optimizePng, optimizeJpg, optimizeSvg));
